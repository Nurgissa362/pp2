...
class Enemy(pygame.sprite.Sprite):
    ...
    def move(self):
        ...
        if self.rect.top > SCREEN_HEIGHT:
            # Increment score when enemy goes off screen
            SCORE += 1
            self.rect.top = 0
            self.rect.center = (random.randint(40, SCREEN_WIDTH - 40), 0)
...
# Coin class (now includes weight)
class Coin(pygame.sprite.Sprite):
    def __init__(self):
        ...
        # Randomly assign weight to the coin
        self.weight = random.randint(1, 5)  # Random weight between 1 and 5

    def reset_position(self):
        self.rect.center = (random.randint(40, SCREEN_WIDTH - 40), random.randint(40, SCREEN_HEIGHT - 200))
        # Reassign random weight for each new coin position
...
while True:
    for event in pygame.event.get():
        if event.type == INC_SPEED:
            # Increase enemy speed every second
            ENEMY_SPEED += 0.5
        if event.type == QUIT:
            pygame.quit()
            sys.exit()
    ...
    if pygame.sprite.spritecollideany(P1, coins):
        # Increase the coins collected by the coin's weight
